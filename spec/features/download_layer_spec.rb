...
feature 'Download layer' do
  ...
  context 'different settings configuration for vector download format options' do
    before do
      Settings.DOWNLOAD_FORMATS.VECTOR = %w[CSV]
    end
    scenario 'options should reflect different Settings configuration' do
      visit solr_document_path('cugir-007950')

      find('#downloads-button').click
      expect(page).to have_css('#downloads-collapse a[data-download-type="csv"]', text: 'Export CSV')
      expect(page).not_to have_css('#downloads-collapse a[data-download-type="kmz"]', text: 'Export KMZ')
    end
  end
end

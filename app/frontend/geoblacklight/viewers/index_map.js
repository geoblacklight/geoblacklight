...
class GeoBlacklightViewerIndexMap extends GeoBlacklightViewerMap {
  ...
  indexMapDownloadTemplate = (data) => data.downloadUrl ? `
...
` : '';
    ...
    fetch(this.data.url)
      ...
      .then(data => {
        const geoJSONLayer = L.geoJson(data, {
          style: feature => this.availabilityStyle(feature.properties.available),
  ...
  updateInformation = (properties) => {
    this.indexMapTemplate(properties).then((updatedProperties) => {
      ...
      const indexMapDownloadHtml = this.indexMapDownloadTemplate(updatedProperties);
      ...
      if(Boolean(downloadElement)) {
        ...
        downloadElement.querySelectorAll('.js-index-map-feature').forEach(elem => elem.remove());
      }
    });
  };
}

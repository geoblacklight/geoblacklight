...
class GeoBlacklightViewerWms extends GeoBlacklightViewerMap {
  ...
  setupInspection() {
    this.map.on('click', async (e) => {
      ...
      spinner.innerHTML = '<tr><td colspan="2"><div class="spinner-border" role="status"><span class="sr-only">Inspecting</span></div></td></tr>';
      ...
      try {
        ...
        if (data.hasOwnProperty('error') || data.values.length === 0) {
          document.querySelector('.attribute-table-body').innerHTML = '<tr><td colspan="2">Could not find that feature</td></tr>';
          return;
        }

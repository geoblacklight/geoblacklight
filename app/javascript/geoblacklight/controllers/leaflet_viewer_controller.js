...
import {
  DEFAULT_BOUNDS,
  DEFAULT_OPACITY,
  DEFAULT_GEOM_OVERLAY_OPTIONS
} from "geoblacklight/leaflet/constants"
...
export default class LeafletViewerController extends Controller {
  ...
  addBoundsOverlay(bounds) {
    const hasMapGeomValue =
      Object.keys(this.mapGeomValue).length > 0 && !this.map.geosearch
    ...
    if (hasMapGeomValue)
      this.map.addLayer(
        L.geoJSON(this.mapGeomValue, overlayOptions[this.pageValue])
      )
  ...
  addInspection() {
    ...
    if (this.protocolValue == "Wms")
      return wmsInspection(
        this.map,
        this.urlValue,
        this.layerIdValue,
        this.previewOverlay
      )
    if (this.protocolValue == "FeatureLayer")
      return featureLayerInspection(this.map, this.previewOverlay)
    if (this.protocolValue == "DynamicMapLayer")
      return dynamicMapLayerInspection(
        this.map,
        this.previewOverlay,
        this.layerIdValue
      )
  ...
  async getPreviewOverlay(protocol, url, options) {
    ...
    if (protocol == "TiledMapLayer")
      return await esriTiledMapLayer(url, options)

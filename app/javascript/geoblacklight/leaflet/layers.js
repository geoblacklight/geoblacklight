...
export const wmtsLayer = async (url, { opacity, detectRetina, layerId }) => {
  ...
  if (!response.ok) throw new Error("Unable to fetch WMTSCapabilities document", url)

  const wmtsCapabilities = await response.text()
  if (!wmtsCapabilities) throw new Error("Unable to parse WMTSCapabilities document", url)
...
export const esriTiledMapLayer = async (url, options) => {
  ...
  if (!response.ok) throw new Error("Unable to fetch EsriTiledMapLayer document", url)
  ...
  if (layerInfo.singleFusedMapCache === true && layerInfo.spatialReference.wkid === 102100) {
...
export const indexMapLayer = async (url, leafletOptions) => {
  ...
  const geoJSONLayer = geoJson(data, {
    style: (feature) => availabilityStyle(feature.properties.available, leafletOptions),

...
export default class sidebarControl extends Control {
  ...
  onAdd(map) {
    ...
    div.addEventListener('click', function(event) {
      const sidebarCollapse = event.target.dataset.target;
      const collapse = event.target.dataset.toggle == 'collapse'
      event.stopPropagation();

      if (sidebarCollapse && collapse){
        document.querySelector(sidebarCollapse).classList.toggle('collapse');
